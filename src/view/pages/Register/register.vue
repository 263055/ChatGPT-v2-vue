      ...
      <header id="header">
        ...
        <div class="content">
          <div class="inner">
            <h1>欢迎来到 4gai </h1>
            <div v-if="isOK">
              <p>如果页面没有任何变化，请点击下方的按钮</p>
              <el-button type="primary" @click="done">注册账号/重置密码</el-button>
            </div>
            <div v-else>
              <p>
                {{ res }}<span> {{ time }}</span>秒后，
                <br>
                网页将会自动跳转至 4gai 的首页！！！
              </p>
            </div>
...
<script>
import {activate, reset} from "@/api/login";
...
#bg:before {
  ...
  background-image: linear-gradient(to top, rgba(3, 10, 33, 0.5), rgba(19, 21, 25, 0.5)), url("/src/assets/overlay.png");
